...
const terminalCommands = {
  ...
  projects: { output: [
    "Live project list is pulled from GitHub — type the section name",
    "or scroll to Projects to see it, since repos change over time.",
  ]},
  achievements: { output: [
...
function mapRepoToProject(repo, index) {
  ...
  const highlights = topics.length
    ? topics.slice(0, 4).map(titleCase)
    : [repo.language ? `${repo.language} Project` : "Public Repository", "Open Source"];
...
function ProjectCard(props) {
  ...
  return e("article", { className: "group hover-card glow-border relative flex flex-col overflow-hidden rounded-2xl border border-[rgba(255,255,255,0.05)] bg-card/40 transition-all duration-300 hover:-translate-y-1 hover:border-cyan/20 hover:shadow-[0_20px_60px_rgba(0,212,255,0.06)]" },
    ...
    e("div", { className: "flex flex-col p-6 md:p-7" },
      e("div", { className: "flex flex-wrap items-start justify-between gap-3" },
        ...
        e("div", { className: "flex items-center gap-2" },
          p.demo && e("a", { href: p.demo, target: "_blank", rel: "noreferrer", className: "flex items-center gap-1.5 rounded-md bg-white/[0.04] px-3 py-1.5 font-mono text-[12px] text-muted transition-all hover:bg-white/[0.08] hover:text-ink" },
      ...
      (p.stars !== undefined || p.updated) && e("div", { className: "mt-5 flex items-center gap-3 border-t border-[rgba(255,255,255,0.05)] pt-4 font-mono text-[11px] text-dim" },
        p.stars !== undefined && e("span", { className: "flex items-center gap-1" },
          e("svg", { width: "12", height: "12", viewBox: "0 0 24 24", fill: "currentColor" },
            e("path", { d: "M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" })
          ), p.stars),
        p.updated && e("span", null, `updated ${timeAgo(p.updated)}`)
      )
    )
  );
}
...
function ProjectCardSkeleton() {
  return e("div", { className: "animate-pulse rounded-2xl border border-[rgba(255,255,255,0.05)] bg-card/30 p-6 md:p-7" },
    e("div", { className: "h-5 w-24 rounded bg-white/[0.06]" }),
    e("div", { className: "mt-5 h-7 w-2/3 rounded bg-white/[0.06]" }),
    e("div", { className: "mt-3 h-4 w-full rounded bg-white/[0.05]" }),
    e("div", { className: "mt-2 h-4 w-5/6 rounded bg-white/[0.05]" }),
    e("div", { className: "mt-6 flex gap-2" },
      e("div", { className: "h-6 w-16 rounded bg-white/[0.05]" }),
      e("div", { className: "h-6 w-20 rounded bg-white/[0.05]" })
    )
  );
}
    ...
    e("div", { className: "mx-auto max-w-6xl px-5" },
      e(ScrollReveal, null,
        e("div", { className: "flex flex-wrap items-center justify-between gap-3" },
          ...
          e("div", { className: "inline-flex items-center gap-2 rounded-full border border-[rgba(255,255,255,0.08)] bg-white/[0.02] px-3 py-1.5 font-mono text-[11px] text-dim" },
            e("span", { className: `h-1.5 w-1.5 rounded-full ${source === "loading" ? "bg-amber animate-pulse" : source === "live" ? "bg-green" : "bg-dim"}` }),
            source === "loading" ? "syncing from GitHub…" : source === "live" ? "live from GitHub" : "cached snapshot",
            e("a", { href: `https://github.com/${GITHUB_USERNAME}`, target: "_blank", rel: "noreferrer", className: "ml-1 text-cyan hover:underline" }, "view all →")
          )
        ),
        e("p", { className: "mt-4 max-w-2xl leading-relaxed text-muted" },
          "Pulled straight from my public GitHub repos, so this list updates automatically as I ship, rename, or retire projects."
        )
      ),
      e("div", { className: "mt-8 columns-1 gap-5 sm:columns-2 lg:columns-3" },
        projects === null
          ? Array.from({ length: 6 }).map((_, i) => e("div", { key: i, className: "mb-5 break-inside-avoid" }, e(ProjectCardSkeleton, null)))
          : projects.map((p, i) => e(ScrollReveal, { key: p.id, delay: Math.min(0.06 * i, 0.36), className: "mb-5 break-inside-avoid" },
              e(TiltCard, null, e(ProjectCard, { project: p }))
            ))
      )
    )
  );
